# Process: Rolling 2023 Test Cases to 2025 and Updating .travis.yml

## Overview

This document describes the end-to-end process used to:
1. Identify 2023 test case entries in `.travis.yml`
2. Query the XBRL US API to get company namespace (fact) data
3. Generate a batch file (`run_2023_roll2025.bat`) to create rolled instance `.xml` files using Arelle + xodel
...
| `original.csv` | `testcase-tools/` | Table of `$EXPECTED`, `file:`, and `updated` (converted URL) columns — derived from `.travis.yml` 2023 entries |
...
## Step 1: Extract 2023 Entries from .travis.yml

Parse `.travis.yml` matrix section for all active (non-`##`) INFILES entries where `$EXPECTED` ends in `2023.xml`. For each `{file: "...", xule_run_only: "..."}` block, capture:
...
- `entity.ticker`
- `report.entity-name`
- `fact` — the `dts.target-namespace` value (company namespace URI, e.g. `http://henryschein.com/20231231`)
...
## Step 4: Generate run_2023_roll2025.bat (Instance Creation)
...
**Command template:**
```bat
./arelleCmdLine.exe --plugins "xodel|EDGAR/transform" -f "{file:}" --xule-time .005 --xule-debug --noCertificateCheck --logFile "D:/DJT/.../tests/input/testfiles/2023-roll-2025/roll{$EXPECTED}" --xule-rule-set "D:/DJT/.../tests/input/2023-roll-2025-V29-test-ruleset.zip" --xule-run-only createInstance,create_fact_values,new-instance-values --logNoRefObjectProperties --xodel-location "D:/DJT/.../tests/input/testfiles/2023-roll-2025/" --xodel-show-xule-log --xince-file-type=xml --xule-arg TAXONOMY_DATE='20251231' --xule-arg PUBLISH_TAXONOMY='{entity.ticker}_taxonomy_2023_2025' --xule-arg TICKER='{entity.ticker}' --xule-arg OLD_CO_NAMESPACE='{fact}' --xule-arg INSTANCE_NAME='{entity.ticker}_instance_2025_2026' --xule-arg NEW_CO_NAMESPACE='{FACTDOMAIN}20261231'
```
...
| `{$EXPECTED}` | `$EXPECTED` (e.g. `DQC.US.0208.10727_WHF-US-2023.xml`) |
...
**Notes:**
- Use double quotes around `"xodel|EDGAR/transform"` — pipe operator breaks batch if single-quoted
- Use forward slashes throughout paths — backslashes with `\t`, `\2` etc. are misinterpreted by Python string literals
- `NEW_CO_NAMESPACE` = `FACTDOMAIN` + `20261231` (target year end for rolled instance)

**Output files per run:**
- `{TICKER}_instance_2023_2025.xml` — the rolled XBRL instance
- `{TICKER}_taxonomy_2023_2025.zip` — the rolled taxonomy package
- `roll{$EXPECTED}` — the Arelle log file (written to `2023-roll-2025/` folder initially, later moved to `tests/output/` after validation)

---

## Step 5: Run run_2023_roll2025.bat

Execute from the directory containing `arelleCmdLine.exe`. Each command fetches the SEC filing via `_htm.xml` URL and produces the instance and taxonomy files in `tests/input/testfiles/2023-roll-2025/`.

---

## Step 6: Generate run_validate_roll.bat (DQC Rule Validation)

For each `roll*.xml` file in `tests/input/testfiles/2023-roll-2025/`, find the corresponding `{TICKER}_instance_2023_2025.xml` and generate a validation command.
...
**Command template:**
```bat
.\arellecmdline.exe --plugins "validate/DQC|EDGAR/transform|inlineXbrlDocumentSet" -f {INSTANCE} -v --xule-run-only {RULE} --xule-time .005 --xule-debug --noCertificateCheck --logFile D:/DJT/.../tests/output/{ROLL} --xule-rule-set D:/DJT/.../dqc_us_rules/dqc-US-2025-V29-ruleset.zip
```

**Placeholder mappings:**
| Placeholder | Value |
|-------------|-------|
| `{INSTANCE}` | `./tests/input/testfiles/2023-roll-2025/{TICKER}_instance_2023_2025.xml` |
| `{RULE}` | Characters from roll filename between `roll` and first `_` (e.g. `DQC.US.0208.10727`) |
| `{ROLL}` | Full roll filename (e.g. `rollDQC.US.0208.10727_WHF-US-2023.xml`) |
...
```
$EXPECTED/DQC.US.xxxx_TICKER-us-2023.xml
```
with:
```
$EXPECTED/rollDQC.US.xxxx_TICKER-us-2023.xml
```
...
- Update `file:` from the original `.htm` SEC URL to the local `./tests/input/testfiles/2023-roll-2025/{TICKER}_instance_2023_2025.xml` path
...
Files in `tests/input/testfiles/2023-roll-2025/` NOT referenced on the active line 22 entry move to `tests/input/testfiles/2023-roll-2025/unused/`:
...
|------|----------|---------|
| `original.csv` | `testcase-tools/` | Source table from `.travis.yml` 2023 entries |
| `results.csv` | `testcase-tools/` | XBRL US API report data |
| `final.csv` | `testcase-tools/` | Merged working table |
| `run_2023_roll2025.bat` | `testcase-tools/` | Creates rolled instance + taxonomy files |
| `run_validate_roll.bat` | `testcase-tools/` | Validates instances against DQC rules |
| `us2023_entries.csv` | `testcase-tools/` | Non-roll DQC.US *-us-2023 entries still in .travis.yml |
| `.travis_copy.yml` | `testcase-tools/` | Working copy of .travis.yml with roll updates applied |
| `{TICKER}_instance_2023_2025.xml` | `2023-roll-2025/` | Rolled XBRL instance files |
| `{TICKER}_taxonomy_2023_2025.zip` | `2023-roll-2025/` | Rolled taxonomy packages |
| `rollDQC.US.*-us-2023.xml` | `tests/output/` | Validation log files (test expected output) |

---

## Notes on File Differences

The `roll*.xml` files in `2023-roll-2025/` are **instance creation logs** (xodel/Xince plugin, large, ~22k lines).
...
| Ruleset | Path |
|---------|------|
| Instance creation | `tests/input/2023-roll-2025-V29-test-ruleset.zip` |
